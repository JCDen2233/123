...
## Этап 7: Смена времени суток

### 7.1 Система времени
- `TimeManager` класс в `static/js/time.js`
- Цикл 24 часа (ускоренный: 1 игровое утро = 30-60 реальных секунд)
- Фазы: рассвет, день, закат, ночь, полночь
- Текущее время: `gameTime` (0-24 часа в формате float)

### 7.2 Освещение и цвета
- Динамическое изменение яркости canvas (globalAlpha/overlay)
- Цвет неба меняется от голубого (день) → оранжевого (закат) → тёмно-синего (ночь)
- Тени становятся длиннее на рассвете/закате
- Ночью — уменьшенная видимость, туман войны

### 7.3 Визуальные эффекты
- Звёзды и луна ночью ( procedural generation на canvas)
- Солнце днём (позиция зависит от времени)
- Окна домов светятся ночью
- Факелы и источники света

### 7.4 Геймплейные изменения
- Некоторые NPC появляются только ночью
- Враги агрессивнее ночью
- Бонусы/штрафы к характеристикам в разное время
- Квесты, зависящие от времени суток

### 7.5 UI времени
- Индикатор текущего времени на HUD (часы)
- Мини-карта показывает фазу дня цветом
- Уведомления о смене фазы ("Наступила ночь!")

### 7.6 Синхронизация
- Сервер хранит глобальное время мира
- Broadcast времени всем клиентам при подключении
- Синхронизация времени через Socket.IO (`time_update` событие)

**Результат этапа:** Полный цикл дня и ночи с визуальными эффектами и геймплейным влиянием.
...
- **`static/js/mapEditor.js`**: выбор тайла из палитры, размещение ЛКМ, удаление ПКМ

### 6.5 Обработка ошибок сети
- Переподключение при обрыве
- Индикатор соединения (connected/disconnected/reconnecting) — визуальный статус в UI
- Буферизация действий при лагах

### 6.6 Финальная полировка
- Плавные переходы UI
- Экран загрузки с прогресс-баром (загрузка ресурсов, подключение к серверу)
- Настройки: громкость звука, чувствительность камеры
- FPS counter для отладки
- Спавн NPC/Mob при инициализации карты
- **Смена времени суток**: цикл день/ночь с изменением освещения, цвета неба и теней
...
| 19 | Редактор карт (mapEditor.js) | □ |
| 20 | Экран загрузки с прогресс-баром | □ |
| 21 | Настройки (громкость, камера) | □ |
| 22 | Индикатор соединения | □ |
| 23 | Спавн NPC/Mob при старте | □ |
| 24 | Смена времени суток (день/ночь) | □ |
